...
export default function NewsletterInsightsPage() {
  ...
  useEffect(() => {
    const fetchNewsletterProfile = async () => {
      try {
        console.log("Newsletter Insights: Fetching newsletter profile...")
        const { data: userData } = await supabase.auth.getUser()

        if (userData?.user) {
          console.log("Newsletter Insights: User found:", userData.user.id)
          ...
          console.log("Newsletter Insights: Profile query result:", profileData, profileError)
          ...
          if (profileError) {
            ...
            console.log("Newsletter Insights: Profile loaded successfully")

            // Check email configuration status for alerts
            if (profileData.from_email_option === "custom_domain") {
              console.log("Newsletter Insights: Custom domain status:", profileData.custom_domain_status)
            }
          } else {
            console.log("Newsletter Insights: No profile found")
    ...
    return (
      <div className="flex items-center justify-center py-16">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading newsletter insights...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="mb-6">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>Newsletter Insights</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
        <h1 className="text-3xl font-bold tracking-tight mt-4">Newsletter Insights</h1>
        <p className="text-muted-foreground mt-1">Track your newsletter performance and engagement metrics</p>
      </div>
