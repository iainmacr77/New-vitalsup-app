-- Simple schema cache refresh without complex concatenation
...
-- Ensure foreign key exists (if missing, add it)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name LIKE '%newsletter_profile_id%'
...
-- Simple table comments to refresh schema cache
COMMENT ON TABLE newsletter_profiles IS 'Newsletter profiles for practices';
COMMENT ON TABLE user_newsletter_roles IS 'User roles for newsletter profiles';
